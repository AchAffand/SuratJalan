...
export const addDeliveryNoteToSupabase = async (note: Omit<DeliveryNote, 'id' | 'createdAt' | 'updatedAt'>): Promise<DeliveryNote> => {
	try {
		...
		const created = dbToDeliveryNote(data);
		// Fire-and-forget push
		broadcastPush({
			title: 'Surat Jalan Baru Dibuat',
			body: `${created.deliveryNoteNumber} untuk ${created.destination}`,
			url: '/pengiriman',
			tag: `delivery-insert-${created.id}`,
			data: { id: created.id, event: 'INSERT' },
		});
		return created;
	} catch (error) {
...
export const updateDeliveryNoteInSupabase = async (id: string, updates: Partial<DeliveryNote>): Promise<DeliveryNote | null> => {
	try {
		...
		const updated = data ? dbToDeliveryNote(data) : null;
		if (updated) {
			broadcastPush({
				title: 'Surat Jalan Diperbarui',
				body: `${updated.deliveryNoteNumber} → ${updated.status}`,
				url: '/pengiriman',
				tag: `delivery-update-${updated.id}`,
				data: { id: updated.id, event: 'UPDATE' },
			});
		}
		return updated;
	} catch (error) {
